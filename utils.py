...
def format_sequence(sequence):
    sequence = re.sub(r'(?<=[^\s0-9])(?=[.,;?])', r' ', sequence)
    sequence = re.sub(r'\((\d+)\)', r'', sequence)
    return re.sub(r'\s\s', ' ', sequence)
...
def get_data(file, filters='!"%;[\\]^_`{|}~\t\n', training_len=50, lower=False, dates='patent_date', subset='patent_abstract'):
    data = pd.read_csv(file, parse_dates=[dates]).dropna(subset = [subset])
    abstracts = [format_sequence(a) for a in list(data[subset])]
    word_idx, idx_word, num_words, word_counts, texts, sequences, features, labels = make_sequences(abstracts, training_len, lower, filters)
